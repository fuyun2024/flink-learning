...
import com.sf.bdp.flink.entity.RowRecord;
 ...
 */
public class Tuple2DeserializationSchema implements DebeziumDeserializationSchema<Tuple2<String, byte[]>> {
     ...
     * @throws Exception
     */
    public void deserialize(SourceRecord sourceRecord, Collector<Tuple2<String, byte[]>> out) throws Exception {
        ...
        if (op == Envelope.Operation.CREATE || op == Envelope.Operation.READ) {
            ...
            rowRecord = RowRecord.buildUpsertRecord(dbName, tableName, RowKind.INSERT,
            ...
            rowRecord = RowRecord.buildDeletedRecord(dbName, tableName, RowKind.DELETE,
                    record.getFieldNames(), record.getFieldTypes(), record.getValues());
        } else {
            record = extractAfterRow(value, valueSchema);
            rowRecord = RowRecord.buildUpsertRecord(dbName, tableName, RowKind.UPDATE_AFTER,
                    record.getFieldNames(), record.getFieldTypes(), record.getValues());
        }

        Tuple2<String, byte[]> tuple2 = new Tuple2<>(dbName + "." + tableName, rowRecord.toString().getBytes(StandardCharsets.UTF_8));
        out.collect(tuple2);
    }

    private RowRecord setDbTable(RowRecord rowRecord, SourceRecord sourceRecord) {
        String[] split = sourceRecord.topic().split("\\.");
        if (split.length != 3) {
            throw new IllegalArgumentException("topic name error : " + sourceRecord.topic());
        }

        rowRecord.setDbName(split[1]);
        rowRecord.setTableName(split[2]);
        return rowRecord;
    }
